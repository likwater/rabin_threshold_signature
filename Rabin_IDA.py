...
def modinv(a, p):
    """Modular inverse of a under modulus p"""
    g, x, _ = egcd(a, p)
    if g != 1:
...
def matrix_mul(A, B, p):
    """Multiply matrices A (m x n) and B (n x k) mod p"""
    m, n = len(A), len(A[0])
    n2, k = len(B), len(B[0])
...
def matrix_inverse(M, p):
    """Compute inverse of square matrix M mod p via Gaussian elimination"""
    n = len(M)
    AM = [row[:] + [1 if i == j else 0 for j in range(n)] for i, row in enumerate(M)]
    for i in range(n):
        pivot = i
        while pivot < n and AM[pivot][i] == 0:
            pivot += 1
        if pivot == n:
            raise ValueError("Matrix is singular and cannot be inverted")
        AM[i], AM[pivot] = AM[pivot], AM[i]
        inv_val = modinv(AM[i][i], p)
        for j in range(2*n):
            AM[i][j] = (AM[i][j] * inv_val) % p
        for r in range(n):
            if r != i:
                factor = AM[r][i]
                for j in range(2*n):
                    AM[r][j] = (AM[r][j] - factor * AM[i][j]) % p
    return [row[n:] for row in AM]

# ----------------------------------------------------------------------------
# IDA: Rabin's Information Dispersal Algorithm with chunking
# ----------------------------------------------------------------------------

def share_secret(secret_bytes, n, k, p=8380417):
    """
    Split secret_bytes into n shares with threshold k using IDA over GF(p).
    - secret_bytes: bytes of any length
    - n: total number of shares
    - k: threshold
    - p: prime modulus (> max byte value)

    Returns (M, shares), where:
    - M: n x k encoding matrix
    - shares: list of n lists, each share is a list of chunks (ints)
    """
    # Convert secret to byte vector
    data = list(secret_bytes)
    # Determine number of blocks
    block_count = (len(data) + k - 1) // k
    # Pad data
    data += [0] * (block_count * k - len(data))
    # Split into blocks
    blocks = [data[i*k:(i+1)*k] for i in range(block_count)]

    # Generate random n x k matrix M with invertible top k rows
    while True:
        M = [[random.randrange(p) for _ in range(k)] for _ in range(n)]
        try:
            _ = matrix_inverse([row[:] for row in M[:k]], p)
            break
        except ValueError:
            continue

    # Compute shares: each share is a list of block_count ints
    shares = [[] for _ in range(n)]
    for blk in blocks:
        # blk is length-k vector
        # compute dot product for each row of M
        dots = [sum(M[i][j] * blk[j] for j in range(k)) % p for i in range(n)]
        for i in range(n):
            shares[i].append(dots[i])

    return M, shares


def reconstruct_secret(shares_subset, M, indices, p=8380417):
    """
    Reconstruct the secret from a subset of shares using IDA.
    - shares_subset: list of shares (each a list of ints) for selected indices
    - M: encoding matrix from share_secret
    - indices: list of row indices for each share in shares_subset (0-based)
    - p: prime modulus

    Returns the reconstructed secret_bytes
    """
    k = len(indices)
    # Extract kxk submatrix
    M_sub = [M[i] for i in indices]
    invM = matrix_inverse(M_sub, p)

    block_count = len(shares_subset[0])
    secret = []
    for b in range(block_count):
        # gather block shares
        share_vec = [shares_subset[i][b] for i in range(k)]
        # reconstruct block
        blk = [sum(invM[row][col] * share_vec[col] for col in range(k)) % p for row in range(k)]
        secret.extend(blk)
...
if __name__ == "__main__":
    ...
    for idx, sh in enumerate(shares):
        print(f"Share {idx}:", sh)

    # Reconstruct from any k shares
    idxs = [0, 1, 3, 5, 7, 8]
    subset = [shares[i] for i in idxs]
    recovered = reconstruct_secret(subset, M, idxs)
