...
class Dilithium:
    ...
    def _h(input_b, l):
        ...
        H: B^*  -> B^*
        """
        return shake256(input_b).read(l)

    def _expand_matrix_from_seed(self, r):
        """
        ...
        for i in range(self.k):
            for j in range(self.l):
                A_data[i][j] = self.R.rejection_sample_ntt_poly(r, i, j)
        return self.M(A_data)

    def _expand_vector_from_seed(self, rho_p):
        s1_elements = [
            self.R.rejection_bounded_poly(rho_p, i, self.eta) for i in range(self.l)
        ]
        s2_elements = [
            self.R.rejection_bounded_poly(rho_p, i, self.eta)
    ...
    def _pack_sk(self, rho, K, tr, s1, s2, t1):
        s1_bytes = s1.bit_pack_s(self.eta)
        s2_bytes = s2.bit_pack_s(self.eta)
        t1_bytes = t1.bit_pack_t0()
        return rho + K + tr + s1_bytes + s2_bytes + t1_bytes
    ...
    def _unpack_sig(self, sig_b):
        c_tilde = sig_b[:32]
        z_bytes = sig_b[32 : -(self.k + self.omega)]
    ...
    def verify(self, pk_bytes, m, sig_b):
        """
