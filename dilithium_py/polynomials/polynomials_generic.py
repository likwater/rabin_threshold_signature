...
class Polynomial:
    ...
    def _sub_mod_q(self, a, y):
        """
        sub two coefficients modulo q
        """
        return (a - y) % self.parent.q

    def _schoolbook_multiplication(self, ot):
        """
    ...
    def _sub_(self, other):
        if isinstance(other, type(self)):
            new_coeffs = [
                self._sub_mod_q(a, y) for a, y in zip(self.coeffs, other.coeffs)
            ]
        elif isinstance(other, int):
            new_coeffs = self.coeffs.copy()
    ...
    def __mul__(self, other):
        if isinstance(other, type(self)):
            ...
            new_coeffs = [(x * other) % self.parent.q for x in self.coeffs]
        else:
            raise NotImplementedError(
