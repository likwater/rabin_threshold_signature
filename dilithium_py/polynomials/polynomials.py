...
class PolynomialRingDilithium(PolynomialRing):
    def __init__(self):
        self.q = 8380417
        self.n = 256
        self.element = PolynomialDilith
        self.element_ntt = NTT
    ...
    def sample_in_ball(self, seeds, tau):
        """
    ...
    def sample_mask_polynomial(self, rho_p, i, kappa, gamma_1):
        """
        ...
        # Initialise the XOF
        seed = rho_p + int.to_bytes(kappa + i, 2, "little")
    ...
    def __bit_unpack(self, input_b, n_bits):
        if (len(input_b) * n_bits) % 8 != 0:
            raise ValueError(
                "Input bytes do not have a length compatible with the bit length"
            )

        r = int.from_bytes(input_b, "little")
        mask = (1 << n_bits) - 1
    ...
    def bit_unpack_z(self, input_b, gamma_1):
        # Level 2 parameter set
        if gamma_1 == (1 << 17):
            altered_coeffs = self.__bit_unpack(input_b, 18)
        # Level 3 and 5 parameter set
        else:
            assert gamma_1 == (
                1 << 19
            ), f"Expected gamma_1 to be either 2^17 or 2^19, got {gamma_1 = }"
            altered_coeffs = self.__bit_unpack(input_b, 20)
...
class PolynomialDilith(Polynomial):
    ...
    def make_hint_optimised(self, other, al):
        coeffs = [
            make_hint_optimised(r, z, al, 8380417)
