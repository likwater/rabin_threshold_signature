...
class ML_DSA:
    ...
    def _expand_vector_from_seed(self, rho_p):
        s1_elements = [
            self.R.rejection_bounded_poly(rho_p, i, self.deta) for i in range(self.l)
        ]
        s2_elements = [
            self.R.rejection_bounded_poly(rho_p, i, self.deta)
    ...
    def _pack_sk(self, rho, K, tr, s1, s2, t0):
        s1_bytes = s1.bit_pack_s(self.deta)
        s2_bytes = s2.bit_pack_s(self.deta)
        t0_bytes = t0.bit_pack_t0()
        return rho + K + tr + s1_bytes + s2_bytes + t0_bytes

    def _sk_list_bytes(self, rho, K, tr, s1, s2, t0):
        s1_bytes = s1.bit_pack_s(self.deta)
        s2_bytes = s2.bit_pack_s(self.deta)
        t0_bytes = t0.bit_pack_t0()
        return (rho, K, tr, s1_bytes, s2_bytes, t0_bytes)
    ...
    def _pack_sig(self, c_t, z, h):
        return c_t + z.bit_pack_z(self.gamma_1) + self._pack_h(h)

    def _unpack_pk(self, pk_b):
        rho, t1_b = pk_b[:32], pk_b[32:]
        t1 = self.M.bit_unpack_t1(t1_b, self.k, 1)
        return rho, t1

    def _unpack_sk(self, sk_b):
        if self.deta == 2:
            s_bytes = 96
        else:
            s_bytes = 128
        ...
        s1 = self.M.bit_unpack_s(s1_bytes, self.l, 1, self.deta)
        s2 = self.M.bit_unpack_s(s2_bytes, self.k, 1, self.deta)
        t0 = self.M.bit_unpack_t0(t0_bytes, self.k, 1)

        return rho, K, tr, s1, s2, t0

    def _unpack_h(self, h_b):
        offsets = [0] + list(h_b[-self.k :])
        non_zero_positions = [
            list(h_b[offsets[i] : offsets[i + 1]]) for i in range(self.k)
        ]
    ...
    def _unpack_sig(self, sig_b):
        c_tilde = sig_b[: self.c_tilde_bytes]
        z_bytes = sig_b[self.c_tilde_bytes : -(self.k + self.omega)]
        h_bytes = sig_b[-(self.k + self.omega) :]
    ...
    def _keygen_internal(self, zeta):
        ...
        sk_list = self._sk_list_bytes(rho, K, tr, s1, s2, t0)

        return pk, sk, sk_list

    def _sign_internal(self, sk_b, m, rnd):
        """
        ...
        rho, K, tr, s1, s2, t0 = self._unpack_sk(sk_b)
    ...
    def keygen(self):
        ...
        pk, sk, sk_list = self._keygen_internal(zeta)
        return pk, sk, sk_list

    def sign(self, sk_b, m, ctx=b"", determin=False):
        """
        ...
        sig_bytes = self._sign_internal(sk_b, m_prime, rnd)
        return sig_bytes

    def verify(self, pk_b, m, sig_b, ctx=b""):
        """
        ...
        return self._verify_internal(pk_b, m_prime, sig_b)
