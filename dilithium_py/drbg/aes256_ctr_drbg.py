...
class AES256_CTR_DRBG:
    ...
    def random_bytes(self, num_bytes: int, addit: Optional[bytes] = None) -> bytes:
        """
        ...
        if addit is None:
            ...
            if len(addit) > self.seed_length:
                raise ValueError(
                    f"The additional input must be of length at most: "
                    f"{self.seed_length}. Input has length {len(addit)}"
                )
            addit += bytes([0]) * (self.seed_length - len(addit))
            self.__ctr_drbg_update(addit)
