class Module:
    ...
    def random_element(self, m, n):
        ...
        """
        elements = [[self.ring.random() for _ in range(n)] for _ in range(m)]
    ...
    def __call__(self, matrix_element, transpose=False):
        if not isinstance(matrix_element, list):
            raise TypeError(
                "elements of a module are matrices, built from elements of the base ring"
            )

        if isinstance(matrix_element[0], list):
            ...
            return self.matrix(self, matrix_element, transpose=transpose)

        elif isinstance(matrix_element[0], self.ring.element):
            if not all(isinstance(aij, self.ring.element) for aij in matrix_element):
                raise TypeError(
                    f"All elements of the matrix must be elements of the ring: {self.ring}"
                )
            return self.matrix(self, [matrix_element], trans=transpose)
    ...
    def vector(self, elements_n):
        ...
        """
        return self.matrix(self, [elements_n], transpose=True)


class Matrix:
    def __init__(self, parents, matrix_datas, trans=False):
        self.parent = parents
        self._data = matrix_datas
        self._transpose = trans
    ...
    def __eq__(self, oth):
        if self.dim() != oth.dim():
            return False
        m, n = self.dim()
        return all([self[i, j] == oth[i, j] for i in range(m) for j in range(n)])
    ...
    def __add__(self, ot):
        ...
        m, n = self.dim()
        return self.parent(
            [[self[i, j] + ot[i, j] for j in range(n)] for i in range(m)],
            False,
        )
